...
import { AlertTriangle, CheckCircle2, ChevronLeft, ChevronRight, CircleDashed, ListTodo, Plus, Search, Sparkles } from "lucide-react";
...
export default function Home() {
  ...
  async function confirmDelete() { if (!deleting) return; try { await actions.remove.mutateAsync(deleting.id); setDeleting(null); } catch {} }
    ...
    <section className="main-area">
      <div className="task-board new-board">
        <section className="workspace queue-panel">
          ...
          <div className="queue-toolbar"><div className="search-box"><Search size={18} /><input value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} placeholder="Tìm công việc hoặc người phụ trách..." />{search !== debouncedSearch && <span className="searching-dot" />}</div><select value={sort} onChange={(e) => { setSort(e.target.value); setPage(1); }}><option value="createdAt-desc">Mới nhất</option><option value="dueAt-asc">Hạn gần nhất</option><option value="title-asc">Tên A → Z</option><option value="updatedAt-desc">Vừa cập nhật</option></select></div>
